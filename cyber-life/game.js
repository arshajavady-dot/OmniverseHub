...
const TILE_SIZE = 16;
const COLS = 120; // 120 * 16 = 1920px wide world
const ROWS = 60;  // 60 * 16 = 960px deep world
...
function generateWorld() {
  ...
  for (let r = 0; r < ROWS; r++) {
    ...
    for (let c = 0; c < COLS; c++) {
      ...
      } else if (r < surfaceRow + 8) {
        row.push(TILES.DIRT);
      } else {
        const rand = Math.random();
        if (rand < 0.06) row.push(TILES.NEON_ORE);
        else if (rand < 0.11) row.push(TILES.PLATINUM);
  ...
  for (let c = 5; c < COLS - 5; c += 6 + Math.floor(Math.random() * 5)) {
...
let particles = [];
let popups = [];
let timeOfDay = 0; // 0 to 1200 (Day/Night cycle)
...
// Enemy Cyber Slime Spawner
function spawnSlime() {
  const x = player.x + (Math.random() > 0.5 ? 250 : -250);
  enemies.push({
    x,
    y: 300,
...
canvas.addEventListener('mousedown', (e) => {
  ...
  if (e.button === 0) { // Left Click: Mine Block or Attack Slimes
...
// World Tile AABB Collision Check
...
function draw() {
  ...
  const isNight = timeOfDay > 600;
  ctx.fillStyle = isNight ? '#050714' : '#0c1a30';
    ...
    for (let c = startCol; c <= endCol; c++) {
      ...
      if (tile !== TILES.AIR) {
        ...
        if (tile === TILES.TORCH) {
          ...
          ctx.beginPath();
          ctx.arc(c * TILE_SIZE + 8, r * TILE_SIZE + 8, 30, 0, Math.PI * 2);
          ctx.fill();
          ctx.restore();
        }
      }
