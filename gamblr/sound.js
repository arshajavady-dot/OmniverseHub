...
class SoundEngine {
  constructor() {
    ...
    // Attach gesture event listeners for instant audio unlock
    const unlockMobile = () => {
      this.init();
    };
  ...
  startCasinoBGM() {
    ...
    // Dm9, G13, Cmaj7, A7b9 jazz-cyber chords
    const chords = [
      [146.83, 174.61, 220.00, 261.63], // Dm9: D3, F3, A3, C4
      [196.00, 246.94, 293.66, 349.23], // G13: G3, B3, D4, F4
      [130.81, 164.81, 196.00, 246.94], // Cmaj7: C3, E3, G3, B3
      [220.00, 277.18, 329.63, 392.00]  // A7: A3, C#4, E4, G4
    ];
    const bassNotes = [73.42, 98.00, 65.41, 110.00]; // D2, G2, C2, A2
    const leadNotes = [587.33, 659.25, 783.99, 880.00, 783.99, 659.25, 587.33, 523.25];
    const stepMs = (60 / 116 / 4) * 1000; // 116 BPM

    this.bgmTimer = setInterval(() => {
      if (this.isMuted || !this.audioCtx) return;
      try {
        const now = this.audioCtx.currentTime;
        const bar = Math.floor(this.bgmStep / 16) % chords.length;
        const chord = chords[bar];
        const bass = bassNotes[bar];

        // 1. Walking Jazz-Cyber Bass
        if (this.bgmStep % 4 === 0) {
        ...
        // 2. Velvet Chord Pad (On start of bar)
        if (this.bgmStep % 16 === 0) {
          chord.forEach(f => {
            const osc = this.audioCtx.createOscillator();
            const gain = this.audioCtx.createGain();
            osc.type = 'sine';
        ...
        // 3. Cyber Shimmer Lead (Every 8th note)
        if (this.bgmStep % 2 === 0) {
          const oscL = this.audioCtx.createOscillator();
          const gainL = this.audioCtx.createGain();
          oscL.type = 'sine';
          oscL.frequency.setValueAtTime(leadNotes[this.bgmStep % leadNotes.length], now);
          gainL.gain.setValueAtTime(0.02, now);
          gainL.gain.exponentialRampToValueAtTime(0.001, now + 0.1);
          oscL.connect(gainL);
          gainL.connect(this.audioCtx.destination);
          oscL.start(now);
          oscL.stop(now + 0.1);
        }

        // 4. Subtle Brush Hi-Hat (Every 16th note off-beat)
        if (this.bgmStep % 2 === 1) {
